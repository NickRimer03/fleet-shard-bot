...
export default () => {
  ...
  async function main() {
    try {
      ...
      // client.setTimeout(main, 60000 - (Date.now() % 60000));
      client.setTimeout(main, 120000);
  ...
  async function sendMessage() {
    ...
    for (let i in mates) {
      let fieldName =
        String(mates[i].time) +
        " - (UTC " +
        String(mates[i].po.hours).padStart(2, "00") +
        ":" +
        String(mates[i].po.minutes).padStart(2, "00") +
        ")";
      let fieldText = "";
